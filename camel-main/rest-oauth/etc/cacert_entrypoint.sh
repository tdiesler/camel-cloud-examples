...
# ------------------------------------------------------------------------------
# Note: This is borrowed from __cacert_entrypoint.sh which is part of the eclipse/temurin:17 base image.
#
# Here we use $CERTIFICATES_DIR instead of the hardcoded /certificates
# ------------------------------------------------------------------------------

set -e

TMPDIR=${TMPDIR:-/tmp}
CERTIFICATES_DIR=${CERTIFICATES_DIR:-/maven/tls}

# JDK truststore location
JRE_CACERTS_PATH=$JAVA_HOME/lib/security/cacerts

# Activated when the certificates dir exits
if [ -d "$CERTIFICATES_DIR" ]; then
    ...
    for i in $CERTIFICATES_DIR/*crt; do
        if [ ! -f "$i" ]; then
            continue
        fi
        tmp_dir=$(mktemp -d)
    ...
    if [ "$(id -u)" -eq 0 ]; then
        ...
        # Copy certificates from $CERTIFICATES_DIR to the system truststore, but only if the directory exists and is not empty.
        ...
        if [ -d $CERTIFICATES_DIR ] && [ "$(ls -A $CERTIFICATES_DIR 2>/dev/null)" ]; then
            cp -La $CERTIFICATES_DIR/* /usr/local/share/ca-certificates/
        fi
