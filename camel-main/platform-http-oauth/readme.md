...
```bash
java -jar target/platform-http-oauth-1.0.0.jar
```
...
```bash
docker build -f src/main/docker/Dockerfile -t platform-http-oauth:1.0.0 .
```

Once the application is published, you can run it directly from the container:

```bash
docker run -it platform-http-oauth:1.0.0
```
