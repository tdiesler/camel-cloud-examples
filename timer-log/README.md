...
First we generated the project's skeleton with `camel-timer`, `camel-log` and `yaml-dsl` enabled. 
Additionally, we add container build and runtime options. In this case, we'd like to build the 
contain image using [Docker](https://quarkus.io/extensions/io.quarkus/quarkus-container-image-docker/) and then deploy it to a local [Minikube](https://quarkus.io/extensions/io.quarkus/quarkus-minikube/) cluster.

```shell
mvn io.quarkus:quarkus-maven-plugin:create \
    -DprojectGroupId=org.acme \
    -DprojectArtifactId=timer-log \
    -Dextensions=camel-quarkus-log,camel-quarkus-timer,camel-quarkus-yaml-dsl,docker,minikube
```
